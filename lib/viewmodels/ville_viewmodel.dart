...
class VilleViewModel extends ChangeNotifier{
  ...
  void _initialiser(){
    _villes = [
      ...
      Ville(nom:'Paris', pays: 'France', temperature: 10, condition: 'Orageux', humidite: 25),
    ];
  ...
  Future<void> selectionnerVille(Ville ville, {bool forcerRafraichissement = false}) async{
    ...
    if (!forcerRafraichissement && entreeCache != null) {
      ...
      if (age < _dureeValiditeCache) {
        ...
        print('[CACHE] ${ville.nom} : utilisation du cache (${age.inMinutes} min)');
    ...
    if(meteo != null){
      ...
      _cacheMeteo[ville.nom] = (meteo, DateTime.now()); // on met a jour le cache
    }else{
